...
class PtSdk
{
    ...
    function __construct($payUserID = '', $paykey = ''){
        require_once dirname(__FILE__).'/ptpay_config.php';
        if ($payUserID && $paykey) {
            $ptpay_config['payUserID'] = $payUserID;
            $ptpay_config['paykey'] = $paykey;
        }
        $this->config = $ptpay_config;
    }
    //创建订单
    ...
    public function filePay($parameter)
    {
        //验证传入的参数
        if ($parameter ==null)
        {
        ...
        $parameter['isHtml']= $this->config['isHtml'];
        $request_url = $this->config['transport'].$this->config['apiurl'];
        $content =get_cur($request_url, $parameter);
        //判断是否是url
        $getOrderInfo = json_decode($content,true);
        if($getOrderInfo['code'] == 1)
        {
            Header("Location:".$this->config['payfile'].$getOrderInfo['data']['orderId']);
        }else
        {
            echo "支付错误，请联系客服:".$content;
        }
        return $content;
    }
    ...
    public function get_WeChat_info($url,$mid)
    {
    ...
    public function getWxInfo($access_token)
    {
        $parameter = array(
            "access_token" => $access_token
        );
        $request_url = $this->config['transport'].$this->config['getWxInfo'];
        $parameter =json_decode(get_cur($request_url,$parameter ),true);
        if($parameter['msg'] =='成功')
        {
