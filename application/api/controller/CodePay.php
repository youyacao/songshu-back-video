...
class CodePay extends Controller
{
    ...
    public function getIntegral(){
        ...
        $payConfig = getPayConfig();
        $gold_rate = $payConfig['gold_rate'] ? $payConfig['gold_rate']:1;
        $money = $gold/$gold_rate;
        $params = [];
        $params['uid']  = $user['id'];
        $params['shop_id']  = 0;
        $params['amount']   = $money;
        ...
        $data = array(
            ...
            "price"     => $money,//金额100元
            "param"     => "",//自定义参数
            "notify_url"=> request()->domain().'/codepay/notifyIntegral',
            "return_url"=> request()->domain().'/codepay/notifyIntegral'
        );
        ksort($data); //重新排序$data数组
        reset($data); //内部指针指向数组中的第一个元素
    ...
    public function getNotifyIntegral() {
        ...
        if (!$_POST['pay_no'] || md5($sign . $codepay_key) != $_POST['sign']) { //不合法的数据
            ...
            $user = Db('user')->where('id', $order_info['uid'])->where('disable', 0)->find();

            $gold_rate = $payConfig['gold_rate'] ? $payConfig['gold_rate']:1;
            $num = intval($money*$gold_rate);
            $allMoney = $num + $user['money'];
            //添加账变记灵
            $data = array();
            ...
            $data['info'] = '充值金币';
            $data['data_id'] = $order_info['id'];
            $data['data_type'] = 'recharge';
            $data['created_at'] = date('Y-m-d H:i:s');
            Db('account_change')->insert($data);
            $res = Db('user')->where('id', $user['id'])->setInc('money', $num);
            exit(json_encode([
                'code'  => 'success',
                'message'   => '成功'
            ]));
